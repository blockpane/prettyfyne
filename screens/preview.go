...
func Preview() *fyne.Container {
	...
	formEntrySelects := widget.NewFormItem("Should we wait?", container.NewHBox(
		selection,
	...
	radio := widget.NewRadioGroup([]string{"Yes", "Not yet", "No thanks, it's 3am!", "I would rather have something else."}, func(s string) {})
	form.Append("Coffee?", container.NewHBox(radio, layout.NewSpacer()))
	...
	return container.New(layout.NewMaxLayout(),
		container.NewScroll(
			container.NewVBox(
				toolbar,
				container.NewHBox(widget.NewHyperlink("Link to prettyfyne", uri), layout.NewSpacer(), boldLabel, layout.NewSpacer(), boldItalicLabel, layout.NewSpacer(), widget.NewLabel("This is a regular label."), layout.NewSpacer(), italicLabel),
				layout.NewSpacer(),
				container.New(layout.NewGridLayout(3), slider, widget.NewIcon(theme.SettingsIcon()),
					layout.NewSpacer(),
					container.NewHBox(layout.NewSpacer(), entryEnabled, pass, entryDisabled, entryHasText, layout.NewSpacer()),
					layout.NewSpacer(),
					container.New(layout.NewGridLayout(3), layout.NewSpacer(), progress, progLabel),
					layout.NewSpacer(),
					container.NewHBox(layout.NewSpacer(), widget.NewButtonWithIcon("Enabled", theme.InfoIcon(), func() {}), monoLabel, buttonDisabled, layout.NewSpacer())),
				layout.NewSpacer(),
				container.NewHBox(layout.NewSpacer(), timeRem, layout.NewSpacer()),
				widget.NewProgressBarInfinite(),
				form,
				layout.NewSpacer(),
				container.NewHBox(layout.NewSpacer(),
					container.New(layout.NewGridWrapLayout(fyne.NewSize(600, 300)),
						multiLineRo,
					),
					layout.NewSpacer(),
				),
